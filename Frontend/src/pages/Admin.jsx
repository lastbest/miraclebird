...
function Admin () {
    ...
    return (
        ...
        <div className={styles.btnCt}>
            <button className={`challengeBtn ${view === 1 ? 'active' : ''}`} onClick={()=>(setView(1))}>챌린지</button>
            <button className={`challengeBtn ${view === 2 ? 'active' : ''}`} onClick={()=>(setView(2))}>신고</button>
            <button className={`reportBtn ${view === 3 ? 'active' : ''}`} onClick={()=>(setView(3))}>민팅</button>
        </div>
